const wwwDomain = process.env.wwwDomain; // wwwDomain: process.env.NODE_ENV === 'production' ? 'https://www.priviatravel.com' : 'https://twww.priviatravel.com'
const mwDomain = process.env.mwDomain;   // mwDomain:  process.env.NODE_ENV === 'production' ? 'https://mw.priviatravel.com' : 'https://tmw.priviatravel.com'
// console.log(`wwwDomain: ${wwwDomain} mwDomain: ${mwDomain}`);  
...
const setCookie = (name, value, expiredays) => {
  ...
  window.document.cookie = name + "=" + escape( value ) + "; expires=" + today.toGMTString() + "; path=/";
};

/* domainUrl */
const domainUrl = (url, isMobile = device.isMobile) => {
  const domainUrl = isMobile ? `${mwDomain}${url}` : `${wwwDomain}${url}`;
  return domainUrl;
}

/* domainMobileMenuUrl */
const domainMobileMenuUrl = (menuDivn) => {
  let domainUrl = mwDomain;
  if (menuDivn === 'air') domainUrl = process.env.NODE_ENV === 'production' ? 'https://m.priviatravel.com/mAirIndex.lts' : 'https://tm.priviatravel.com/mAirIndex.lts';
  if (menuDivn === 'hotel') domainUrl = process.env.NODE_ENV === 'production' ? 'https://mhotel.priviatravel.com' : 'https://muiux.priviatravel.com:444';  
  return domainUrl;
}
